...
describe('usePricingPlanStyleProps', () => {
  ...
  it('should return class with --comparable when isComparable is true', () => {
    const { result } = renderHook(() => usePricingPlanStyleProps({ isComparable: true }));

    expect(result.current.classProps.root).toContain('PricingPlan--comparable');
    expect(result.current.props).toStrictEqual({});
  });
