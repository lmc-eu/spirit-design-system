...
describe('useButtonStyleProps', () => {
  ...
  it('should return asymmetrical classes when all breakpoints are false', () => {
    const props = {
      color: 'primary',
      size: 'medium',
    ...
    expect(result.current.classProps).toBe(
      'Button Button--primary Button--medium Button--asymmetrical Button--tablet--asymmetrical Button--desktop--asymmetrical',
    );
  });
