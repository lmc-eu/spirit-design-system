...
```jsx
import { ButtonLink, Icon, VisuallyHidden } from '@alma-oss/spirit-web-react';
...
To turn off symmetrical at a specific breakpoint, set the value to `false`:
...
visible text. Using `display: none` utility classes (like `d-tablet-none`) hides content from screen readers, so
the `VisuallyHidden` component ensures the label is always accessible regardless of viewport size.
...
```jsx
import { ButtonLink, Grid } from '@alma-oss/spirit-web-react';
