...
Deploying serious web applications to production on a regular basis requires automation.
Running testing, linting, and a build before deployment is a accepted practice to ensure a quality product is deployed.
...
We can run this automation on our own machines during development, but it can be easy to forget to do this.
It's even harder to be confident the automation was run when you are trying to combine the work of multiple people (who's responsible to run the deploy script?).
...
It does not require an additional account, though it does necessitate you use GitHub for hosting the code.
...
We have a single action that handles running linting, type checking, tests, and another for deployment to both staging (preview branchech) and production (`main` branch).

## Consequences

This means users of the Spirit Design System can monitor its quality publicly without the need to have theirs own account.
They can also alert if any integration is badly done or disfunction.
