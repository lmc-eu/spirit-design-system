"use strict";const j={SHORT:"short",LONG:"long"};function L(t,n=j.SHORT){const i=t.match(/.{1,2}/g);let u=n===j.SHORT;return i&&i.forEach(o=>{n===j.SHORT&&(u=/^(.)\1+$/.test(o))}),u?`${t.substring(0,1)}${t.substring(2,3)}${t.substring(4,5)}${t.substring(6,7)}`:n===j.LONG&&t.length===3?t.split("").map(o=>o+o).join(""):t}function E(t){return t.a<255?`#${t.hex}`:`#${L(t.hex.substring(0,6))}`}function W(t,n){const i=n.y-t.y,u=n.x-t.x;let l=Math.atan2(i,u)*180/Math.PI;return l+=90,(l<0?360+l:l)%360}function T(t){return t.replace(/\s/g,"-").replace(/\//g,"-").replace(/-\d\d-/g,"-").replace(/--+/g,"-").toLowerCase()}function w(t){return t.split("-").map(n=>n.charAt(0).toUpperCase()+n.slice(1).toLowerCase()).join("")}function S(t){return t.split("-").map((n,i)=>i>0?n.charAt(0).toUpperCase()+n.slice(1).toLowerCase():n).join("")}function G(t){return t.slice(-1)==="s"?t.replace(/.$/,""):t}function M(t,n){let i="";const u=[];return Object.entries(t).forEach(([o,l])=>{const m=S(o);u.push(m),l.length>0&&(i=`${i}
export const ${m} = {
${l.map($=>`    ${S($)}: ${S(o)}${w($)},`).join(`
`)}
};
...
`),i}function d(t,n){let i=t.toString();return+t!=0&&(n==="Pixels"&&(i+="px"),n==="rem"&&(i+="rem")),i}function I(t,n){const i=T(t.origin?t.origin.name:t.name),u=T(n.origin?n.origin.name:n.name);return i.localeCompare(u)}function R(t,n,i,u){const o=i.trim().split(",");let l="",m="";if(o.some(p=>((t.origin?t.origin.name:t.name).includes(p)&&(l=p),(n.origin?n.origin.name:n.name).includes(p)&&(m=p),!1)),u&&!l)return-1;let $=o.indexOf(l)-o.indexOf(m);return $===0&&($=I(t,n)),$}function D(t,n){return+t.value.text-+n.value.text}function z(t){return t.name.match(/ \d$/)}function F(t,n,i=[],u=!1,o=!1,l="",m=""){let $=n.sort((e,a)=>{if(u){const f=e.name.match(/\d+$/),r=a.name.match(/\d+$/);if(f&&r)return parseInt(f[0],10)-parseInt(r[0],10)}return o?D(e,a):I(e,a)});l.length>0&&($=n.sort((e,a)=>R(e,a,l,o)));const p=[],v={};$.forEach(e=>{if(m.length>0&&!e.name.startsWith(m))return;let a=T(e.name);e.origin&&!e.origin.name.toLowerCase().startsWith("gradients/gradient")&&(a=T(e.origin.name));const f=z(e);let r="";!f&&i.length>0&&i.forEach(g=>{Object.values(g.tokenIds).indexOf(e.id)>-1&&g.isRoot===!1&&(r=G(T(g.name)))});const y=a.split("-"),h=r===""?y[0]:r,C=r===""?a.replace(`${y[0]}-`,""):a.replace(`${r}-`,"");v[h]&&v[h].length>0?!f&&y[0]!==a?v[h].push(C):v[h]=[]:!f&&y[0]!==a?v[h]=[C]:v[h]=[];let c="";if(e.tokenType==="Color")c=E(e.value);else if(e.tokenType==="Radius")c=d(e.value.radius.measure,e.value.radius.unit);else if(e.tokenType==="GenericToken")e.propertyValues.unitless?c=e.value.text:c=d(e.value.text,"Pixels");else if(e.tokenType==="Shadow")c=`${d(e.value.x.measure,e.value.x.unit)} ${d(e.value.y.measure,e.value.y.unit)} ${d(e.value.radius.measure,e.value.radius.unit)} ${d(e.value.spread.measure,e.value.spread.unit)} ${E(e.value.color)}`;else if(e.tokenType==="Gradient"){let g="linear-gradient",O=`${Math.round(W(e.value.from,e.value.to)*100)/100}deg`;e.value.type==="Radial"&&(g="radial-gradient",O="circle at center"),c=`${g}(var(--gradient-angle, ${O}), ${e.value.stops.map(x=>`${E(x.color)} ${Math.round(x.position*10)/10*100}%`).join(", ")})`}else e.tokenType==="Border"?c=e.propertyValues.style??d(e.value.width.measure,e.value.width.unit):c=d(e.value.measure,e.value.unit);if(f){const g=T(e.name.replace(/ \d$/,"")),O=p.filter(P=>P.startsWith(`$${g}: `))[0],x=p.indexOf(O);x>-1&&(p[x]=p[x].replace(/= '(.*)';/g,`= '${c}, $1';`))}else p.push(`export const ${S(a)} = ${c==="0"?0:`'${c}'`};`)});const s=$.length===0?"":M(v,t);return`${p.join(`
`)}
