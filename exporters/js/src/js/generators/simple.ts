...
export function generateSimple(
  ...
  tokens.forEach((token) => {
    ...
    } else if (!groupToken && split[0] !== name) {
    ...
    if (groupToken) {
      ...
      if (index > -1) {
        vars[index] = vars[index].replace(/= '(.*)';/g, `= '${value}, $1';`);
      }
    } else {
      vars.push(`export const ${kebabCaseToCamelCase(name)} = ${value === '0' ? 0 : `'${value}'`};`);
    }
  });

  const typesPrint = tokens.length === 0 ? '' : printTypes(types, globalName);
